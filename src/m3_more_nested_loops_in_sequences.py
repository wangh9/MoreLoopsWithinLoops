...
def run_test_largest_number():
    ...
    expected = 10
    answer = largest_number(([10], [1,1,2,4,6,3,3,2], [9]))
    print('Expected and actual are:', expected, answer)
...
def run_test_largest_negative_number():
    ...
    expected = -3
    answer = largest_negative_number(([1], [1], [-3]))
    print('Expected and actual are:', expected, answer)

    expected = None
    answer = largest_negative_number(([1], [1], [1]))
    print('Expected and actual are:', expected, answer)

    expected = -1
    answer = largest_negative_number(([-1], [-5], [-10,1,2,3]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for k in range(len(seq_seq)):
        for b in range(len(seq_seq[k])):
            if seq_seq[k][b] > big and seq_seq[k][b] < 0:
                big = seq_seq[k][b]
...
def first_is_elsewhere_too(seq_seq):
    ...
    for k in range(len(seq_seq[0])):
        for b in range(1,len(seq_seq)):
            for c in range(len(seq_seq[b])):
                if seq_seq[b][c] == seq_seq[0][k]:
                    return True
    return False
